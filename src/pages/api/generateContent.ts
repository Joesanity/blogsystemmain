...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { keywords, stockCategory, locations, phoneNumber, emailAddress, companyName } = req.body;

  try {
    // Title of content
    const titleResponse = await openai.chat.completions.create({
      messages: [
        { role: 'system', content: 'You are an expert at coming up with unique blog titles based on the keywords provided. You will only print out the title, and no other information at all as a response, in British English' },
        { role: 'user', content: `Create one blog post idea from one of these: ${keywords} and use one of these locations: ${locations}. Make it suitable for long tail keywords, it should be between 7-10 words and be a creative idea that isn't common.` }
      ],
      model: 'gpt-4o-mini',
    });
